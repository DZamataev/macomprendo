    ...
    static let russian = FactoryPresetContent(
        ...
        entries: [
            ...
            .translate: .init(name: "Перевести", template: """
                Целевой язык: {chosen_language}
                Переведи следующий текст на этот язык. Сохрани тон и форматирование.
                {instruction}
            ...
            .translateAndOrganize: .init(name: "Перевести и систематизировать", template: """
                Целевой язык: {chosen_language}
                Переведи следующий текст на этот язык, а затем систематизируй результат: сгруппируй \
                связанные мысли, добавь короткие заголовки или список там, где это облегчает чтение, убери \
                повторы. Не выдумывай фактов и не теряй информацию.
                {instruction}
            ...
            .keyActions: .init(name: "Действия", template: """
                ...
                {text}
                """),
            .briefTranslated: .init(name: "Кратко, с переводом", template: """
                Целевой язык: {chosen_language}
                Изложи следующий текст в двух-трёх предложениях на этом языке, на каком бы языке ни был \
                исходный текст.
                {instruction}

                {text}
                """),
            .bulletsTranslated: .init(name: "Списком, с переводом", template: """
                Целевой язык: {chosen_language}
                Изложи следующий текст не более чем шестью краткими пунктами, по одной строке каждый, каждый \
                начинается с «- ». Пиши на этом языке, на каком бы языке ни был исходный текст.
                {instruction}

                {text}
                """),
            .tldrTranslated: .init(name: "TL;DR, с переводом", template: """
                Целевой язык: {chosen_language}
                Дай TL;DR следующего текста одним предложением на этом языке, на каком бы языке ни был \
                исходный текст.
                {instruction}

                {text}
                """),
            .keyActionsTranslated: .init(name: "Действия, с переводом", template: """
                Целевой язык: {chosen_language}
                Выпиши конкретные задачи из следующего текста нумерованным списком на этом языке, на каком \
                бы языке ни был исходный текст. Если задач нет, ответь ровно «Задач нет».
                {instruction}
