...
- SPM deps ONLY: whisper.cpp (consumed as an xcframework binary target; `import whisper`), `sindresorhus/KeyboardShortcuts` (from 2.0.0), `phosphor-icons/swift`. In-app icons go through `Icon(_:size:)`, never `import PhosphorSwift`. The menubar status item stays an SF Symbol (`waveform`).
...
- `UI/Components/Icon.swift`: `enum AppIcon: String` (cases `microphone, waveform, speakerHigh, stop, play, sparkle, textAa, clipboardText, arrowSquareIn, copy, gear, keyboard, downloadSimple, trash, checkCircle, warningCircle, x, plus, minus, arrowsClockwise, cloud, cpu, listBullets, magicWand`) and `Icon(_ icon: AppIcon, size: CGFloat)`, rendered from vendored Phosphor SVGs. **Never `import PhosphorSwift` — always use this seam.**
...
struct HUDView: View {
    ...
    private var content: some View {
        ...
        case .success(let message):
            label(message, icon: .checkCircle, tint: .accentColor)
        case .error(let message):
            label(message, icon: .warningCircle, tint: .red)
        case .toast(let message):
            label(message, icon: .warningCircle, tint: .secondary)
        }
    }
...
struct OnboardingView: View {
    ...
    private var step: some View {
        ...
        case .ollama:
            stepBody(icon: .cpu,
                ...
                case .some(true):
                    Label { Text("Ollama is running.") } icon: { Icon(.checkCircle, size: 14) }
    ...
    private var modelStateRow: some View {
        ...
        case .downloaded:
            Label { Text("Downloaded.") } icon: { Icon(.checkCircle, size: 14) }
                .foregroundStyle(.green)
        case .failed(let message):
            Label { Text(message) } icon: { Icon(.warningCircle, size: 14) }
                .foregroundStyle(.red)
        }
    }
    ...
    private func statusRow(_ status: PermissionStatus) -> some View {
        switch status {
        case .granted:
            Label { Text("Granted.") } icon: { Icon(.checkCircle, size: 14) }
                .foregroundStyle(.green)
        case .denied:
            Label { Text("Denied — turn Macomprendo on in System Settings, then come back.") }
                ...
                .foregroundStyle(.orange)
        case .undetermined:
            Label { Text("Not asked yet.") } icon: { Icon(.minus, size: 14) }
                .foregroundStyle(.secondary)
        }
    }
...
struct SettingsView: View {
    var body: some View {
        TabView {
            GeneralTab()
                .tabItem { Label { Text("General") } icon: { Icon(.gear, size: 16) } }
            HotkeysTab()
                .tabItem { Label { Text("Hotkeys") } icon: { Icon(.keyboard, size: 16) } }
        }
...
private struct ModelsTabContent: View {
    ...
    private func stateView(_ row: ModelsViewModel.Row) -> some View {
        ...
        case .downloaded:
            HStack(spacing: 8) {
                Label { Text("Ready") } icon: { Icon(.checkCircle, size: 14) }
                    .foregroundStyle(.green)
...
```swift
            ...
            ModelsTab()
                .tabItem { Label { Text("Models") } icon: { Icon(.downloadSimple, size: 16) } }
```
...
private struct ProvidersTabContent: View {
    ...
    private var list: some View {
        VStack(spacing: 8) {
            ...
            HStack {
                Button { viewModel.add() } label: { Icon(.plus, size: 12) }
                Button {
...
```swift
            ProvidersTab()
                .tabItem { Label { Text("Providers") } icon: { Icon(.cloud, size: 16) } }
```
